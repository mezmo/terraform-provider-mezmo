		...
		Steps: []resource.TestStep{
			// Error: pipeline_id is required
			{
				Config: SetCachedConfig(cacheKey, `
					...
					resource "mezmo_encrypt_fields_processor" "my_processor" {
						fields = [".nope"]
					}`,
			...
			// Create with defaults
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestMatchResourceAttr(
						"mezmo_encrypt_fields_processor.my_processor", "id", regexp.MustCompile(`[\w-]{36}`)),

					StateHasExpectedValues("mezmo_encrypt_fields_processor.my_processor", map[string]any{
						"pipeline_id":   "#mezmo_pipeline.test_parent.id",
						"title":         "encrypt fields title",
						"description":   "encrypt fields desc",
						"generation_id": "0",
