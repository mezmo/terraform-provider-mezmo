...
var testAccProtoV6ProviderFactories = map[string]func() (tfprotov6.ProviderServer, error){
	"mezmo": providerserver.NewProtocol6WithError(provider.New("processors_test")()),
}
...
func testAccBackend(resourceName string, userConfig map[string]any) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		processor, err := client.Processor(rs.Primary.Attributes["pipeline_id"], rs.Primary.ID, ctx)
		if err != nil {
			return err
		}

		// Check the backend UserConfig matches the passed userConfig
		return providertest.ValidateUserConfig(processor.UserConfig, userConfig)
	}
}
