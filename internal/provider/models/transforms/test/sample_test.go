...
func TestSampleTransform(t *testing.T) {
		...
		Steps: []resource.TestStep{
			// Error: pipeline_id is required
			{
				Config: SetCachedConfig(cacheKey, `
					...
					resource "mezmo_sample_transform" "my_transform" {
						field = ".nope"
					}`,
			...
			// Create with defaults
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestMatchResourceAttr(
						"mezmo_sample_transform.my_transform", "id", regexp.MustCompile(`[\w-]{36}`)),

					StateHasExpectedValues("mezmo_sample_transform.my_transform", map[string]any{
						"pipeline_id":   "#mezmo_pipeline.test_parent.id",
						"title":         "test title",
						"description":   "test desc",
						"generation_id": "0",
