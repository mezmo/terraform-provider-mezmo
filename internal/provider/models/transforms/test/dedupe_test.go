		...
		Steps: []resource.TestStep{
			// Error: pipeline_id is required
			{
				Config: SetCachedConfig(cacheKey, `
					...
					resource "mezmo_dedupe_transform" "my_transform" {
						fields = [".nope"]
					}`,
			...
			// Create with defaults
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestMatchResourceAttr(
						"mezmo_dedupe_transform.my_transform", "id", regexp.MustCompile(`[\w-]{36}`)),

					StateHasExpectedValues("mezmo_dedupe_transform.my_transform", map[string]any{
						"pipeline_id":      "#mezmo_pipeline.test_parent.id",
						"title":            "dedupe title",
						"description":      "dedupe desc",
						"generation_id":    "0",
