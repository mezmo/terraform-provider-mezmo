...
func TestDecryptFieldsTransform(t *testing.T) {
		...
		Steps: []resource.TestStep{
			// Error: pipeline_id is required
			{
				Config: SetCachedConfig(cacheKey, `
					...
					resource "mezmo_decrypt_fields_transform" "my_transform" {
						fields = [".nope"]
					}`,
			...
			// Create with defaults
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestMatchResourceAttr(
						"mezmo_decrypt_fields_transform.my_transform", "id", regexp.MustCompile(`[\w-]{36}`)),

					StateHasExpectedValues("mezmo_decrypt_fields_transform.my_transform", map[string]any{
						"pipeline_id":   "#mezmo_pipeline.test_parent.id",
						"title":         "decrypt fields title",
						"description":   "decrypt fields desc",
						"generation_id": "0",
