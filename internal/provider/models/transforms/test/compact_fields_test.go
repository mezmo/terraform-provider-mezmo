...
func TestCompactFieldsTransform(t *testing.T) {
		...
		Steps: []resource.TestStep{
			// Error: pipeline_id is required
			{
				Config: SetCachedConfig("compact_fields_resources", `
					...
					resource "mezmo_compact_fields_transform" "my_transform" {
						fields = [".nope"]
					}`,
			...
			// Create with defaults
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestMatchResourceAttr(
						"mezmo_compact_fields_transform.my_transform", "id", regexp.MustCompile(`[\w-]{36}`)),

					StateHasExpectedValues("mezmo_compact_fields_transform.my_transform", map[string]any{
						"pipeline_id":    "#mezmo_pipeline.test_parent.id",
						"title":          "compact fields title",
						"description":    "compact fields desc",
						"generation_id":  "0",
