...
func TestMezmoSinkResource(t *testing.T) {
	const cacheKey = "mezmo_sink_resources"
		...
		Steps: []resource.TestStep{
			// Error: pipeline_id is required
			{
				Config: GetProviderConfig() + `
					resource "mezmo_logs_sink" "my_sink" {
			...
			// Create test defaults
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestMatchResourceAttr(
						"mezmo_logs_sink.my_sink", "id", regexp.MustCompile(`[\w-]{36}`)),

					StateHasExpectedValues("mezmo_logs_sink.my_sink", map[string]any{
						"pipeline_id":   "#mezmo_pipeline.test_parent.id",
						"title":         "my sink",
						"description":   "my sink description",
						"generation_id": "0",
