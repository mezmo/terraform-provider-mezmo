...
func TestKafkaSinkResource(t *testing.T) {
		...
		Steps: []resource.TestStep{
			// Error: pipeline_id is required
			{
				Config: SetCachedConfig(cacheKey, `
					...
					resource "mezmo_kafka_sink" "my_sink" {
						itle = "my kafka title"
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestMatchResourceAttr(
						"mezmo_kafka_sink.my_sink", "id", regexp.MustCompile(`[\w-]{36}`)),
					StateHasExpectedValues("mezmo_kafka_sink.my_sink", map[string]any{
						"pipeline_id":     "#mezmo_pipeline.test_parent.id",
						"title":           "my kafka title",
						"description":     "my kafka description",
						"generation_id":   "0",
