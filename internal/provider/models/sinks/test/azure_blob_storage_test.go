...
func TestAzureBlobStorageSinkResource(t *testing.T) {
	const cacheKey = "azure_blob_sink_resources"
		...
		Steps: []resource.TestStep{
			// Error: properties are required
			{
				Config: GetProviderConfig() + `
					resource "mezmo_azure_blob_storage_sink" "my_sink" {
						inputs      = ["abc"]
						pipeline_id = "pip1"
			...
			// Create test defaults
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestMatchResourceAttr(
						"mezmo_azure_blob_storage_sink.my_sink", "id", regexp.MustCompile(`[\w-]{36}`)),

					StateHasExpectedValues("mezmo_azure_blob_storage_sink.my_sink", map[string]any{
						"pipeline_id":        "#mezmo_pipeline.test_parent.id",
						"title":              "my sink",
						"description":        "my sink description",
						"generation_id":      "0",
