...
func TestSplunkHecLogsSinkResource(t *testing.T) {
	const cacheKey = "splunk_hec_logs_sink_resources"
		...
		Steps: []resource.TestStep{
			// Error: properties are required
			{
				Config: GetProviderConfig() + `
					resource "mezmo_splunk_hec_logs_sink" "my_sink" {
						inputs   = ["abc"]
			...
			// Create test defaults
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestMatchResourceAttr(
						"mezmo_splunk_hec_logs_sink.my_sink", "id", regexp.MustCompile(`[\w-]{36}`)),

					StateHasExpectedValues("mezmo_splunk_hec_logs_sink.my_sink", map[string]any{
						"pipeline_id":            "#mezmo_pipeline.test_parent.id",
						"title":                  "my sink",
						"description":            "my sink description",
						"generation_id":          "0",
